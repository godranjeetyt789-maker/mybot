    ...
    with _conn() as c:
        ...
        defaults = {
            ...
            "deposit_text":  (
                ...
                "👇 Niche link pe click karke deposit karo:"
            ),
            "welcome_text":  (
                ...
                "✅ Niche diye saare channels join karo\n"
                "🔓 Phir <b>Verify &amp; Unlock</b> button dabao\n\n"
                "👇 <b>Sabse pehle yahan join karo:</b>"
            ),
            "joined_text":   (
                ...
                "━━━━━━━━━━━━━━━━━━━━━\n\n"
                "👇 Deposit karne ke liye niche click karo:"
            ),
            "channels":      "[]",
            "admin_ids":     "",
        }
...
def kb_not_joined(channels):
    """Premium style — 2 clean Join buttons per row + lock unlock button"""
    rows = []
    pair = []
    for i, ch in enumerate(channels):
        label = f"Join ↗"
        pair.append(InlineKeyboardButton(label, url=ch["link"]))
        if len(pair) == 2:
            rows.append(pair)
            pair = []
    if pair:
        rows.append(pair)
    rows.append([InlineKeyboardButton("🔐 Verify & Unlock Reward 🎁", callback_data="verify_join")])
    return InlineKeyboardMarkup(rows)

def kb_after_join(channels):
    """Premium style after join — channel links + big deposit CTA"""
    rows = []
    pair = []
    for ch in channels:
    ...
    if pair:
        rows.append(pair)
    rows.append([InlineKeyboardButton("💸 Deposit Now & Earn Reward 🎁", callback_data="deposit")])
    return InlineKeyboardMarkup(rows)
...
async def send_joined_content(bot, chat_id, user_id, cfg):
    """
    Premium 3-step unlock flow:
      Step 1 — 🔓 Reward Unlock animation message (short suspense)
      Step 2 — Voice note with Sound On caption
      Step 3 — Main content: joined text + channel links + Deposit CTA
    """
    voice_id    = cfg.get("voice_file_id", "")
    joined_text = cfg.get("joined_text",
        "🔊 <b>Sound On Karo — Important Message Sun Lo!</b> 👆\n\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        "⚠️ <b>Deposit Milega — Play on Sound</b> 🎙️👆👆\n"
        "━━━━━━━━━━━━━━━━━━━━━\n\n"
    ...
    # ── Step 1: Unlock animation message ─────────────────
    unlock_msg = await bot.send_message(
        chat_id=chat_id,
        text=(
            "🔓 <b>Reward Unlock Ho Raha Hai...</b>\n\n"
            "⏳ Please wait..."
        ),
    ...
    # ── Step 2: Voice note with premium caption ───────────
    if voice_id:
        try:
            await bot.send_voice(
                ...
                voice=voice_id,
                caption=(
                    "🔊 <b>Sound On Karo</b> — Yeh message zaroor suno! 🎙️👆"
                ),
                parse_mode=ParseMode.HTML,
            )
    ...
    # ── Step 3: Main joined content + Deposit CTA ─────────
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── NOOP (decorative separator button) ───────────────
    if data == "noop":
        await q.answer()
        return
    ...
    if data == "verify_join":
        ...
        if not_joined:
            ...
            # ── Step 1: Purana message DELETE karo ──
            await q.answer()  # loader hatao, koi popup nahi
            try:
                await q.message.delete()
            except Exception:
                pass

            # ── Step 2: Screen pe NAYA fresh message bhejo ──
            cfg2 = cfg_get()
            base_text = cfg2.get("welcome_text",
                "🔐 <b>Hey {name}! Reward Unlock Karo</b>\n\n"
                "✅ Niche channels join karo aur reward pao!"
            ).format(name=user.first_name or "User")
        ...
        # ── Sab join ho gaya — unlock flow shuru ──
        await q.answer("🔓 Reward Unlock Ho Raha Hai...", show_alert=False)
        try:
            await q.message.delete()
        except Exception:
            pass
    ...
    # ── DEPOSIT ──────────────────────────────────────────
        ...
        rows.append([InlineKeyboardButton("🚀 Deposit Karo & Reward Pao 💰", url=link)])
