...
def kb_not_joined(channels):
    ...
    rows = []
    # Pair banao — 2 buttons ek row mein
    pair = []
    for ch in channels:
        pair.append(InlineKeyboardButton(f"📢 {ch.get('name','Join')}", url=ch["link"]))
        if len(pair) == 2:
            rows.append(pair)
            pair = []
    if pair:  # bacha hua odd channel
        rows.append(pair)
    rows.append([InlineKeyboardButton("✅ Join Kar Liya – Verify Karo", callback_data="verify_join")])
    return InlineKeyboardMarkup(rows)

def kb_after_join(channels):
    """2 Join buttons per row, phir Deposit Now"""
    rows = []
    pair = []
    for ch in channels:
        pair.append(InlineKeyboardButton(f"📢 {ch.get('name','Join')}", url=ch["link"]))
...
async def send_joined_content(bot, chat_id, user_id, cfg):
    """Voice + text + buttons — sab ek saath"""
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "verify_join":
        ...
        if not_joined:
            ...
            await q.answer(f"❌ In channels ko join karo pehle:\n{names}", show_alert=True)
            return

        # Sab join kar liya — purana msg delete karo, naya bhejo
        try:
    ...
    # ── DEPOSIT ──────────────────────────────────────────
        ...
        await q.edit_message_text(dep_text, reply_markup=InlineKeyboardMarkup(rows), parse_mode=ParseMode.HTML)
