"""
╔══════════════════════════════════════════════════════════════╗
║   PREMIUM FORCE-JOIN BOT  —  DUAL CHANNEL SET SUPPORT       ║
...
def _pair_buttons(channels, url_key="link", label_fn=None):
    """Helper — 2 buttons per row from a channel list."""
    rows, pair = [], []
    for ch in channels:
        label = label_fn(ch) if label_fn else "Join ↗"
...
def kb_force_join(not_joined_channels):
    """Screen 1 — force join keyboard."""
    rows = _pair_buttons(not_joined_channels)
    rows.append([InlineKeyboardButton(
        "🔐 Verify & Unlock Reward 🎁", callback_data="verify_join")])
    return InlineKeyboardMarkup(rows)

def kb_after_join(deposit_link):
    """Screen 2 — after join keyboard: Set-2 channels + Deposit button."""
    after_chs = get_channels("channels_after")
    rows = _pair_buttons(after_chs, label_fn=lambda ch: f"📢 {ch.get('name','Channel')}")
    rows.append([InlineKeyboardButton(
        "🚀 Deposit Karo & Reward Pao 💰",
...
# ═══════════════════════════════════════════════════════════════
#  CHANNEL LIST DISPLAY (reusable for both sets)
# ═══════════════════════════════════════════════════════════════
    ...
    if not channels:
        await q.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"➕ Add Channel", callback_data=add_cb)],
...
# ═══════════════════════════════════════════════════════════════
#  PREMIUM UNLOCK FLOW  (called after all Set-1 channels joined)
# ═══════════════════════════════════════════════════════════════

async def send_joined_content(bot, chat_id, user_id, cfg):
    """
    Premium 3-step reveal:
      1️⃣  Unlock animation message
      2️⃣  Voice note (with sound-on caption)
      3️⃣  Joined text  +  Set-2 channels  +  Deposit button
    """
    ...
    sent = await bot.send_message(
        chat_id=chat_id,
        text=joined_text,
        reply_markup=kb_after_join(deposit_link),
        parse_mode=ParseMode.HTML,
    )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not_joined:
        ...
        sent = await update.message.reply_text(
            text,
            reply_markup=kb_force_join(not_joined),
            parse_mode=ParseMode.HTML,
        )
        db_set_msg(user.id, sent.message_id)
    else:
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "verify_join":
        ...
        if not_joined:
            ...
            # delete old, send fresh message with only remaining channels
    ...
    # ════════════════════════════════════════════════════
        ...
        # ── BROADCAST ─────────────────────────────────────
            ...
            await q.edit_message_text(
                "📨 Broadcast message type karo (HTML allowed):",
                reply_markup=kb_back(), parse_mode=ParseMode.HTML)
